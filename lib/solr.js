 ...
 * @param {String} handler name (default: select) - optional 
 * @param {Function} callback(err,obj) - a function executed when the Solr server responds or an error occurs
 * @param {Error} callback().err
 * @param {Object} callback().obj - JSON response sent by the Solr server deserialized
 *
...
Client.prototype.search = function(query, handlerName, callback){
   var self = this;
   ...
   this.options.fullPath = [this.options.path,this.options.core, (handlerName || 'select') + '?' + parameters + '&wt=json']
                              .filter(function(element){
                                 if(element) return true;
                                 return false;
                              })
